...
if( global.oldtvfilter_enabled ) {
    ...
    */
    ...
    //  Shadow  FOR THE FUTURE
        ...
        INTRELACAMENTO??
        surface_set_target(global.bufferSurface[surfaceFlag]);
            shader_set(sh_OLDTVFilter_Ghost);
                shader_set_uniform_f(global._ghost_width, 1.0/(window_width*surface_aspect));
                shader_set_uniform_f(global._ghost_height, 1.0/(window_height*surface_aspect));
                texture_set_stage(global._ghost_lastframe, surface_get_texture(global.bufferLastFrame));
                draw_surface(global.bufferSurface[!surfaceFlag], 0, 0);
            shader_reset();
        surface_reset_target();
    
        surface_set_target(global.bufferLastFrame);
        draw_surface_stretched(application_surface, 0, 0, window_width, window_height);
        surface_reset_target(); 
        ...
        */
        /*
        FANTASMA?? (legal para o filtro do GameBoy)
    ...
    if( global.noise_enabled ) {
        texture_set_repeat_ext(global._noise_pattern, true);
        texture_set_repeat_ext(global._static_pattern, true);
        texture_set_repeat_ext(global._dirt_pattern, true);
    ...
    if( global.chromatic_enabled ) {
        texture_set_repeat_ext(global._chromatic_pattern, true);
    ...
    if( global.scanline_enabled ) {
        texture_set_repeat_ext(global._scanline_pattern, true);
